...
namespace App\Reports\Teknik\ProyeksiBangun;
...
class ProyeksiBangunReport extends \koolreport\KoolReport {
    ...
    function setup()
    {
        $this->teknikproyeksibanguntable1();
    }

    function teknikproyeksibanguntable1() {
        $node = $this->src('sqlDataSources');
        $node->query("
            EXEC sp_boc_proyeksi_bangun @rt_period = :end_date, @project_no = :project
        ")
        ->params(array(
        ...
        ->pipe(new Map(array(
            '{value}' => function($row, $metaData) {
                $row['SPK_KELUAR_INFRASTRUKTUR'] > 0 ? $row['SPK_KELUAR_INFRASTRUKTUR'] = $row['SPK_KELUAR_INFRASTRUKTUR'] / 1000000 : $row['SPK_KELUAR_INFRASTRUKTUR'] = 0;
                $row['CASHOUT_INFRASTRUKTUR'] > 0 ? $row['CASHOUT_INFRASTRUKTUR'] = $row['CASHOUT_INFRASTRUKTUR'] / 1000000 : $row['CASHOUT_INFRASTRUKTUR'] = 0;
        ...
        ->pipe($this->dataStore('teknik_proyeksi_bangun_table1'));
    }
}