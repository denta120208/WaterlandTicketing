...
namespace App\Reports\FinanceAccounting\AgingScheduleEscrow;
...
class AgingScheduleEscrowReport extends \koolreport\KoolReport {
    ...
    function setup()
    {
        $this->aging_schedule_escrow_table();
        $this->aging_schedule_escrow_excel_table();
    }

    function aging_schedule_escrow_table() {
        ...
        $node->query("
            EXEC sp_boc_aging_schedule_escrow_bank @rt_period = :cut_off, @project_no = :project
        ")
        ->params(array(
        ...
        ->pipe(new ColumnMeta(array(
            ...
            "DAYS_0_UNTIL_30_NOMINAL"=>array(
                "label" => "Rp",
                "footer"=>"sum",
                "footerText"=>"<b>@value</b>",
            ),
            "DAYS_0_UNTIL_30_UNIT"=>array(
                "label" => "Unit",
                "footer"=>"sum",
                "footerText"=>"<b>@value</b>",
            ),
            "DAYS_31_UNTIL_60_NOMINAL"=>array(
                "label" => "Rp",
                "footer"=>"sum",
                "footerText"=>"<b>@value</b>",
            ),
            "DAYS_31_UNTIL_60_UNIT"=>array(
                "label" => "Unit",
                "footer"=>"sum",
                "footerText"=>"<b>@value</b>",
            ),
            "DAYS_61_UNTIL_90_NOMINAL"=>array(
                "label" => "Rp",
                "footer"=>"sum",
                "footerText"=>"<b>@value</b>",
            ),
            "DAYS_61_UNTIL_90_UNIT"=>array(
                "label" => "Unit",
                "footer"=>"sum",
                "footerText"=>"<b>@value</b>",
            ),
            "DAYS_OVER_90_NOMINAL"=>array(
                "label" => "Rp",
                "footer"=>"sum",
                "footerText"=>"<b>@value</b>",
            ),
            "DAYS_OVER_90_UNIT"=>array(
                "label" => "Unit",
                "footer"=>"sum",
                "footerText"=>"<b>@value</b>",
            ),
        ...
        ->pipe(new Map(array(
            '{value}' => function($row, $metaData) {
                $row['BANK'] = "<p style='text-align: left;'>".$row['BANK']."</p>";
        ...
        ->pipe($this->dataStore('finance_accounting_aging_schedule_escrow_table'));
    }

    function aging_schedule_escrow_excel_table() {
        // $project_id = $this->params["project"];
        // $business_id = DB::select("SELECT * FROM MD_PROJECT WHERE PROJECT_NO_CHAR = $project_id");
        ...
        ->pipe($this->dataStore('finance_accounting_aging_schedule_escrow_excel_table'));
    }
}
