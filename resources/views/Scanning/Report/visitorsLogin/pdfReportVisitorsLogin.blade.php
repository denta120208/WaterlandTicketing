@extends('Scanning.Report.visitorsLogin.pdfcetak')
            ...
            <div>
                <center>
                    <div class="headerbawahtengah">
                        <h1><b><label>Visitors Login</label></b></h1>
                        <?php
        ...
        <table class="stripe hover compact" id="data_bill_payment" cellspacing="0" width="100%">
            <thead>
                <tr>
                    ...
                    <th>Total Qty</th>
                    <th>Total Login</th>
                    <th>Total Not Login</th>
                </tr>
            </thead>
            <tbody>
            @if(count($dataVisitorsLogin) > 0)
            <?php $i = 1; $qty = 0; $qtyFree = 0; $totalQty = 0; $totalLogin = 0; $totalNotLogin = 0; ?>
            @foreach($dataVisitorsLogin as $data)
                <tr>
                    <td style="text-align: left">{{ $i }}</td>
                    <td style="text-align: center">{{ date('d-m-Y', strtotime($data['TGL'])) }}</td>
                    <td style="text-align: right">{{ $data['QTY_TICKET_INT'] }}</td>
                    <td style="text-align: right">{{ $data['QTY_TICKET_FREE_INT'] }}</td>
                    <td style="text-align: right">{{ $data['TOTAL_QTY_TICKET'] }}</td>
                    <td style="text-align: right">{{ $data['IS_LOGIN'] }}</td>
                    <td style="text-align: right">{{ $data['IS_NOT_LOGIN'] }}</td>
                </tr>
                <?php $i++; $qty += $data['QTY_TICKET_INT']; $qtyFree += $data['QTY_TICKET_FREE_INT']; $totalQty += $data['TOTAL_QTY_TICKET']; $totalLogin += $data['IS_LOGIN']; $totalNotLogin += $data['IS_NOT_LOGIN']; ?>
            @endforeach
            <tr>
                    <td style="text-align: left; font-size: 120%;"><b>TOTAL</b></td>
                    <td style="text-align: center; font-size: 120%;"><b>-</b></td>
                    <td style="text-align: right; font-size: 120%;"><b>{{ $qty }}</b></td>
                    <td style="text-align: right; font-size: 120%;"><b>{{ $qtyFree }}</b></td>
                    <td style="text-align: right; font-size: 120%;"><b>{{ $totalQty }}</b></td>
                    <td style="text-align: right; font-size: 120%;"><b>{{ $totalLogin }}</b></td>
                    <td style="text-align: right; font-size: 120%;"><b>{{ $totalNotLogin }}</b></td>
                </tr>
            @else
            <tr>
