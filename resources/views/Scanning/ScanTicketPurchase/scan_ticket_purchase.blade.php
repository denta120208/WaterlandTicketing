...
@section('navbar_header')
    Scan Ticket Purchase - <b>{{session('current_project_char')}}</b>
@endsection
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="row" style="padding-bottom: 5%;">
                        <div class="col-sm-6">
                            <form action="{{ url('change_camera_setting') }}" method="post">
                                {{ csrf_field() }}
                                <div class="form-group">
                                    ...
                                    <select id="CAM_SETTING" name="CAM_SETTING" class="form-control select2" style="width: 100%;" required>
                                        <option value="CAMERA_DISABLED" {{ $dataCameraSetting->SETTING == "CAMERA_DISABLED" ? "selected" : ""}}>Turn Off Camera</option>
                                        <option value="user" {{ $dataCameraSetting->SETTING == "user" ? "selected" : ""}}>Kamera Depan</option>
                                        <option value="environment" {{ $dataCameraSetting->SETTING == "environment" ? "selected" : ""}}>Kamera Belakang</option>
                                    </select>
                                </div>
                            </form>
                        </div>
                        <div class="col-sm-6">
                            <form action="{{ url('input_qr_number') }}" method="post">
                                {{ csrf_field() }}
                                <div class="form-group">
                    ...
                    @if($dataCameraSetting->SETTING != "CAMERA_DISABLED")
                    <form>
                        <div id="reader"></div>
                    </form>
                    @endif
...
<script src="{{asset('scanningqrcode/html5-qrcode-v2.3.4.min.js')}}" type="text/javascript"></script>
