...
@section('navbar_header')
    Report Revenue By Promo - <b>{{session('current_project_char')}}</b>
@endsection

@section('header_title')
    Report Revenue By Promo
@endsection
                            ...
                            <div class="col-sm-4">
                                <div class="form-group">
                                    ...
                                    <select id="ddlKategori" name="ddlKategori" class="form-control select2" style="width: 100%;" required>
                                        ...
                                        <option value="ALL" {{ $KATEGORI_PARAM == "ALL" ? "selected" : "" }}>ALL</option>
                                        <option value="Ticket" {{ $KATEGORI_PARAM == "Ticket" ? "selected" : "" }}>Ticket</option>
                                        <option value="Equipment" {{ $KATEGORI_PARAM == "Equipment" ? "selected" : "" }}>Equipment</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md" style="overflow-x:auto;">
                                    <div style='text-align: center; margin-bottom:30px;'>
                                        ...
                                        <a target="_blank" class="btn btn-danger" href="{{ URL('/view_report_rev_ticket_by_promo_print/'.base64_encode($START_DATE_PARAM).'/'.base64_encode($END_DATE_PARAM).'/'.base64_encode($KATEGORI_PARAM)) }}" onclick="window.open(this.href).print(); return false">
                                            Print
                                        </a>
                                    </div>
                                </div>
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <?php
                                                DataTables::create(array(
                                                    "name" => "reportTable1",
                                                    "dataSource"=>$report->dataStore("revenue_ticket_by_promo_table1"),
                                                    "themeBase"=>"bs4",
                                                    "showFooter" => true,
                                                    "cssClass"=>array(
                                                        "table"=>"table table-striped table-bordered",
                                                        "td"=>function($row, $colName) {                                                            
                                                            if($colName == "PROMO_DESC_CHAR") {
                                                                return "dt-nowrap";
                                                            }
                                                    ...
                                                    "columns" => array(
                                                        ...
                                                        "PROMO_DESC_CHAR" => ["label" => "Promo", "formatValue" => function($value, $row) { return $value; }, "footerText" => "<b>-</b>"],
...
<script type="text/javascript">
    ...
    function getExcel() {
        ...
        var url = '{{ url("view_report_rev_ticket_by_promo_excel/start_date_param/end_date_param/kategori_param") }}';
    ...
    function getSubmit() {
        ...
        if(isValid == false) {
            ...
            var end_date = btoa(document.getElementById("txtEndDate").value);
            var kategori = btoa(document.getElementById("ddlKategori").value);
            var url = '{{ url("view_report_rev_ticket_by_promo/start_date_param/end_date_param/kategori_param") }}';
            url = url.replace('start_date_param', start_date);
            url = url.replace('end_date_param', end_date);
            url = url.replace('kategori_param', kategori);
            window.location.href = url;
