...
@section('navbar_header')
    Report Revenue By Payment Method - <b>{{session('current_project_char')}}</b>
@endsection
                            ...
                            <div class="row">
                                <div class="col-md" style="overflow-x:auto;">
                                    <div style='text-align: center; margin-bottom:30px;'>
                                        ...
                                        <a target="_blank" class="btn btn-danger" href="{{ URL('/view_report_rev_ticket_by_payment_method_print/'.base64_encode($START_DATE_PARAM).'/'.base64_encode($END_DATE_PARAM).'/'.base64_encode($KATEGORI_PARAM)) }}" onclick="window.open(this.href).print(); return false">
                                            Print
                                        </a>
                                    </div>
                                </div>
...
<script type="text/javascript">
    ...
    function getExcel() {
        ...
        var url = '{{ url("view_report_rev_ticket_by_payment_method_excel/start_date_param/end_date_param/kategori_param") }}';
    ...
    function getSubmit() {
        ...
        if(isValid == false) {
            ...
            var end_date = btoa(document.getElementById("txtEndDate").value);
            var kategori = btoa(document.getElementById("ddlKategori").value);
            var url = '{{ url("view_report_rev_ticket_by_payment_method/start_date_param/end_date_param/kategori_param") }}';
            url = url.replace('start_date_param', start_date);
            url = url.replace('end_date_param', end_date);
            url = url.replace('kategori_param', kategori);
            window.location.href = url;
