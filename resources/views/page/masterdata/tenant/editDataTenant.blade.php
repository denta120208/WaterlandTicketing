...
@section('navbar_header')
    Form Edit Data Tenant - <b>{{session('current_project_char')}}</b>
@endsection
...
<script type="text/javascript">
    function delItem(id){
        $.ajax({
            type: "post",
            url: "{{ route('masterdata.tenant.deleteitemaddresstenant') }}",
            data: {MD_TENANT_TAX_ID_INT:id, _token: "{{ csrf_token() }}"},
            dataType: 'json',
            cache: false,
    ...
    function getItem(id){
        $.ajax({
            type: "post",
            url: "{{ route('masterdata.tenant.getitemaddresstenant') }}",
            data: {MD_TENANT_TAX_ID_INT:id, _token: "{{ csrf_token() }}"},
            dataType: 'json',
            cache: false,
            beforeSend: function(){ $('#loading').modal('show'); },
    ...
    $(function(){
        $('#update').on('click',function(){
            ...
            else
            {
                $.ajax({
                    type: "post",
                    url: "{{ route('masterdata.tenant.saveaddresstenant') }}",
...
<div class="container-xxl">
    <div class="row justify-content-center">
        <div class="col-md">
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route('masterdata.tenant.editdatatenant') }}">
                        <fieldset>
                            ...
                            <div class="row" style="padding-left: 5px;">
                                <div class="col-md-12">
                                    <table class="table-striped table-hover compact" id="engineering_table" cellspacing="0" width="100%">
                                        ...
                                        <tbody>
                                        <?php $i = 1;?>
                                        @foreach($dataAddressTax as $data)
                                            <tr>
                                                <td>{{$i}}</td>
