...
@section('navbar_header')
    Form Util Billing Unapproval - <b>{{session('current_project_char')}}</b>
@endsection

@section('header_title')
    Form Util Billing Unapproval
@endsection
...
<script type="text/javascript">
    function delItem(id){
        $.ajax({
            type: "post",
            url: "{{ route('engineering.unapproveutilbilling') }}",
            data: {
                ID_BILLING:id,
                _token: "{{ csrf_token() }}"
            },
                    ...
                    <div class="row" style="padding-left: 5px;">
                        <div class="col-md-12">
                            <table class="table table-bordered table-hover dataTable  display compact" id="billingpayment_report_summary" style="width: 100%">
                                ...
                                <tbody>
                                    <?php $i = 1;?>
                                    @foreach($utilsBillingApprove as $data)
                                        <tr>
                                            <td>{{$i}}</td>
                                            ...
                                            <td style="text-align:center;">
                                                <i class='fa fa-arrow-left' title='Unapprove Item' onclick='delItem(<?php echo $data->ID_BILLING; ?>);'></i>
                                            </td>
                                            <?php $i++;?>
                                        </tr>
                                    @endforeach
