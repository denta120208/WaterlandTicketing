...
@section('navbar_header')
    Form List Transaction Faktur - <b>{{session('current_project_char')}}</b>
@endsection
...
<div class="modal fade" id="edit-trx-code-modal" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
        ...
        <form method="POST" action="{{route('tf.listTransactionFakturEditTrxCode')}}">
            @csrf
...
<div class="container-xxl">
    <div class="row justify-content-center">
        <div class="col-md">
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route('accounting.tax.viewlisttransaksifaktur') }}">
                        @csrf
                    ...
                    @if($dataFaktur == 1)
                    <form method="POST" action="{{ route('accounting.tax.exportdatafaktur') }}">
                        @csrf
                        ...
                        <div class="row" style="padding-left: 5px;">
                            <div class="col-md-12">
                                <table class="table table-bordered table-hover dataTable  display compact" id="tax_invoice_table" style="width: 100%">
                                    ...
                                    <tbody>
                                    <?php $i = 1;?>
                                    @foreach($dataTransFaktur as $data)
                                        <tr>
                                            ...
                                            @if($data->INVOICE_FP_NOCHAR == "")
                                            <td></td>
                                            @else
                                            <td><a href="javascript:void(0)" onclick="editDataTrxCode('<?php echo $data->INVOICE_TRANS_ID_INT ?>','<?php echo $data->KODE_PAJAK.''.$data->PEMBETULAN_PAJAK ?>','<?php echo $data->INVOICE_FP_NOCHAR ?>')">{{ $data->KODE_PAJAK }}{{ $data->PEMBETULAN_PAJAK }}</a></td>
                                            @endif
                                            <td>{{ $data->INVOICE_FP_NOCHAR }}</td>
                                            <td>{{ $data->TGL_SCHEDULE_DATE }}</td>
                                            <td>{{ $data->INVOICE_TRANS_DESC_CHAR }}</td>
                                            <td>{{ $data->MD_TENANT_NAME_CHAR }}</td>
                                            <td>{{ $data->MD_TENANT_NIK }}</td>
                                            <td>{{ $data->MD_TENANT_NPWP }}</td>
                                            <td>{{ $data->MD_TENANT_ADDRESS_TAX }}</td>
