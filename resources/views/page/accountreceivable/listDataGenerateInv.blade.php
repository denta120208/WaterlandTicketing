...
@section('navbar_header')
    Form Generate Invoice - <b>{{session('current_project_char')}}</b>
@endsection
...
<div class="modal fade" id="edit-desc-modal" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
        ...
        <form method="POST" action="{{route('gi.generateInvoiceEditDesc')}}">
            @csrf
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <div class="col-sm-12">
                        ...
                        <input type="hidden" class="form-control" name="CUT_OFF_POST" id="CUT_OFF_POST" style="width: 100%;" placeholder="Cut Off" value="{{empty($dateCutOffReal) ? NULL : $dateCutOffReal}}" readonly required />
                        <input type="hidden" class="form-control" name="TYPE_POST" id="TYPE_POST" style="width: 100%;" placeholder="Type" value="{{empty($dataCatgory) ? NULL : $dataCatgory}}" readonly required />
...
<div class="container-xxl">
    <div class="row justify-content-center">
        <div class="col-md">
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route('invoice.viewlistgenerateinvoice') }}">
                        @csrf
                    ...
                    <div class="row" style="padding-left: 5px;">
                        <div class="col-sm-12">
                            <b>Cut Off Date : {{$dateCutOff}}</b><br>
                        </div>
                    </div>
                    <div class="row" style="padding-left: 5px;">
                        <div class="col-md-12">
                            @if($dataGenerateInv <> 0 && ($dataCatgory == 'Rental'))
                                <form action="{{ URL::route('invoice.generateinvoicerental') }}" method="post">
                                    @csrf
                                    ...
                                    <table class="table-striped table-hover compact" id="tdp_report" cellspacing="0" width="100%">
                                        ...
                                        @foreach($dataInvoice as $data)
                                            <tr>
                                                ...
                                                @if($data->MD_TENANT_PPH_INT == 1)
                                                    <td>Potong Tenant</td>
                                                @elseif($data->MD_TENANT_PPH_INT == 0)
                                                ...
                                                <td class="center">
                                                    <a class="btn btn-sm btn-info" href="{{ URL('/invoice/printinvoiceperforma/' . $data->PSM_SCHEDULE_ID_INT.'/'.$cutoff) }}" onclick="window.open(this.href).print(); return false">
                                                        <i>
                                                            Print Proforma
                                                        </i>
                                                    </a>
                            ...
                            @elseif($dataGenerateInv <> 0 && ($dataCatgory == 'SecurityDeposit'))
                                <form action="{{ URL::route('invoice.generateinvoicesecuritydesposit') }}" method="post">
                                    @csrf
                                    <input type="hidden" name="cutoff" value="<?php echo $cutoff; ?>" class="form-control" readonly>
                                    <br><br>
                                    <div class="row">
                            ...
                            @elseif($dataGenerateInv <> 0 && ($dataCatgory == 'ServiceCharge'))
                                <form action="{{ URL::route('invoice.generateinvoiceservicecharge') }}" method="post">
                                    @csrf
                                    <input type="hidden" name="cutoff" value="<?php echo $cutoff; ?>" class="form-control" readonly>
                                    <br><br>
                                    <div class="row">
                                    ...
                                    <table class="table-striped table-hover compact" id="tdp_report" cellspacing="0" width="100%">
                                        ...
                                        @foreach($dataInvoice as $data)
                                            <tr>
                                                ...
                                                <td style="text-align: right;">{{number_format($data->BILL_AMOUNT,0,'','')}}</td>
                                                <td class="center">
                                                    <a class="btn btn-sm btn-info" href="{{ URL('/invoice/printinvoiceperformaservicecharge/' . $data->PSM_SCHEDULE_ID_INT.'/'.$cutoff) }}" onclick="window.open(this.href).print(); return false">
                                                        <i>
                                                            Print Proforma
                                                        </i>
                                                    </a>
                            ...
                            @elseif ($dataGenerateInv <> 0 && $dataCatgory == 'Utility')
                                <form action="{{ URL::route('invoice.generateinvoiceutility') }}" method="post">
                                    <input type="hidden" name="cutoff" value="<?php echo $cutoff; ?>" class="form-control" readonly>
                                    <br><br>
                                    <div class="row">
                                        <div class="col-lg-2">
                                    ...
                                    <table class="table-striped table-hover compact" id="tdp_report1" cellspacing="0" width="100%">
                                        ...
                                        @foreach($dataInvoice as $data)
                                            <tr>
                                                ...
                                                <td style="text-align: right;">{{number_format($data->DPP,0,'','')}}</td>
                                                <td style="text-align: right;">{{number_format($data->PPN,0,'','')}}</td>
                                                <td style="text-align: right;">{{number_format($data->TOTAL,0,'','')}}</td>
                                                <td class="center">
                                                    <a class="btn btn-sm btn-info" href="{{ URL('/invoice/printinvoiceperformautility/' . $data->ID_BILLING.'/'.$cutoff) }}" onclick="window.open(this.href).print(); return false">
                                                        <i>
                                                            Print Performa
                                                        </i>
                                                    </a>
                            ...
                            @elseif($dataGenerateInv <> 0 && ($dataCatgory == 'CasualLeasing'))
                                <form action="{{ URL::route('invoice.generateinvoicecasual') }}" method="post">
                                    <input type="hidden" name="cutoff" value="<?php echo $cutoff; ?>" class="form-control" readonly>
                                    <br><br>
                                    <div class="row">
                                        <div class="col-lg-2">
                            ...
                            @elseif($dataGenerateInv <> 0 && ($dataCatgory == 'Others'))
                                <form action="{{ URL::route('invoice.generateinvoiceothers') }}" method="post">
                                    <input type="hidden" name="cutoff" value="<?php echo $cutoff; ?>" class="form-control" readonly>
                                    <br><br>
                                    <div class="row">
                                        <div class="col-lg-2">
