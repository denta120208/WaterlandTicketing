...
@section('navbar_header')
    Form Rental Equipment - <b>{{session('current_project_char')}}</b>
@endsection
        ...
        <div class="col-md">
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ url('save_rental_equipment') }}" onsubmit="return validateAmountNotSameForm()" method="post">
                        {{ csrf_field() }}
                        <div class="row">
                            <div class="col-sm-2">
                            ...
                            <div class="col-sm-4">
                                <div class="form-group">
                                    ...
                                    <select id="DDL_EQUIPMENT" name="DDL_EQUIPMENT[]" class="form-control select2" multiple="multiple" data-placeholder="Select Equipment" style="width: 100%;" required>                                        
                                        @foreach($ddlDataEquipment as $data)
                                        <option value="{{ $data->MD_EQUIPMENT_ID_INT }}" {{ $data->IS_RENT == 1 ? "disabled" : "" }}>
                                            {{ $data->MD_EQUIPMENT_CATEGORY_DESC_CHAR }} - {{ $data->EQUIPMENT_ASSET_NUMBER }} {{ $data->IS_RENT == 1 ? "(Disabled)" : "" }}
                                        </option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <a href="{{ route('rentEquipment') }}" class="btn btn-danger float-right" style="margin-right: 10px;">Back to List</a>
                    </form>
                </div>
            </div>
        </div>
